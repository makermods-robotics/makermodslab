...
- [remote_inference.py](makermodslab/remote_inference.py) — remote inference (DRTC): the arm is driven locally while the policy runs on a remote GPU, the two meeting over a LiveKit room. This process owns **only** the robot side; it verifies the SFU and that an operator is present before energizing, and never launches the GPU itself. Since S3.8 the Lab CAN launch the GPU — [modal_launcher.py](makermodslab/modal_launcher.py), three v1 routes under `/api/v1/remote-inference/gpu` — but as a **Lab-level resource beside the session, not inside it**: it shells out to the attached `modal` CLI, holds no hardware, resolves its room and credentials through the session's own `resolve_transport()` (never a second credential path), keeps the API secret out of argv by passing it in the child env, stops itself after 10 idle minutes, and neither stops a session on its exit nor is stopped by one — its `ready` is a log-derived hint, and `_probe_room` remains the only gate on energizing the arm. The SFU itself is the Lab's own since S3.6 ([sfu.py](makermodslab/sfu.py), `makermodslab --sfu`, a launcher child so it outlives `--reload`): when it is running the session takes the url, the room and the child's **token** from it in-process and reads no credential file, and `~/.cache/huggingface/lerobot/livekit.env` is the LiveKit Cloud fallback. [drtc_protocol.py](makermodslab/drtc_protocol.py) is the stdin/stdout line protocol between it and the child, its own module for the same reason `eval_protocol.py` is — the parent must never import `livekit.portal` (an FFI dylib behind the optional `[drtc]` extra) and the child is exactly the process that does. `drtc/` holds the child entrypoints and the LiveKit plumbing; nothing there is imported at server boot. Only two v1 routes are its own (`/remote-inference-status`, `/remote-inference/transport`) — start and stop ride the sessions surface as kind `remote_inference`.
