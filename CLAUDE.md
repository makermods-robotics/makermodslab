...
- Both kinds are in the mutex matrix (`robot.busy.hosting` / `robot.busy.remote_teleoperation`, reciprocal checks in every feature) and in `STARTABLE_KINDS`. Portal is the optional `remote` extra (`makermodslab[remote]`), imported lazily; without it both kinds refuse with `system.extra_missing`. Its PyPI wheel is tagged cp312 + Linux x86_64/aarch64 + Apple Silicon — a packaging artifact (the lib is a ctypes cdylib), to be fixed in our Portal fork.
