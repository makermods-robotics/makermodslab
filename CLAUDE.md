...
- `robots/*.json` — per-robot records: `arm_type` (read back verbatim; an id the registry does not know lists as `arm_available: false`), arm layout (`mode: single|bimanual` with right-arm fields), ports, cameras, calibration names, `motor_power`
...
Adding a robot type means adding an `ArmFamily` module under [arms/](makermodslab/arms/) and registering it: the factory, the calibration flows, port detection, the pre-torque preflight, every stop path and rollout resolve the family from `arm_type` and never branch on it themselves. Two things still differ per family by construction rather than through a family method, and are gated by the family's flags: the SO-101's range-sweep flows (`calibrate.py` / `auto_calibrate.py`, chosen by `supports_auto_calibration` / `uses_zero_calibration` in sessions.py) and rollout's port-based Feetech preflights (the subprocess owns the port, so they open it themselves).
