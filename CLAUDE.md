...
Frontend checks (run from `frontend/`): `npm run lint`, `npx tsc -b`, `npm run build`. Some type/lint errors pre-date any given change — record the baseline before you start and compare against it; don't fix pre-existing errors unrelated to your change.

**Type-check with `npx tsc -b`, never `npx tsc --noEmit`.** `frontend/tsconfig.json` is a solution-style root — `"files": []` plus references to `tsconfig.app.json` (`src`) and `tsconfig.node.json` (`vite.config.ts`). Plain `tsc --noEmit` does not follow project references, so it checks zero files and exits 0 no matter what is broken; it was silently a no-op in the docs and in CI for as long as both existed. Build mode follows the references. Both child projects already set `noEmit: true`, so `-b` emits nothing but `*.tsbuildinfo` (already in `frontend/.gitignore`).
