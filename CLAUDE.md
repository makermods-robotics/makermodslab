...
- [remote_host.py](makermodslab/remote_host.py) — session kind `hosting`, the STATION side, and **station mode** (`makermodslab --host <robot>`, requires `--sfu`): the process hosts that robot from startup with no browser and no lease, and re-arms hosting after any local session. Opens the record's follower(s) WITH the record's cameras, joins this node's room as Portal's `Robot` through the `lerobot-teleoperator-livekit` plugin, publishes state + frames and executes the SEATED operator's actions. **Parked and engaged:** parked = torque off at the rest pose, streaming, listening; engaged = following. Parked → engaged automatically when an operator takes the seat (or on an `engage` RPC after a Home), always through a soft start from the follower's present pose; engaged → parked on the operator's Home (holds until Engage), on `release` (the operator ending its session — immediate), on a silent loss after `GRACE_S` (15 s; a reconnect with the same identity resumes), on an action-stream stall of `GRACE_S`, and on the station's stop — each through teleoperate.py's return-to-rest, then torque off. Re-energizing writes the present pose as the goal BEFORE enabling torque (the stale-Goal_Position snap, see dagger_runner._restore_torque). **Single seat:** the robot token caps the room at 2 participants, the token route refuses a second operator token while the seat is held (`sfu.seat_taken`), operators never self-claim — `SeatMonitor` (pure, clock-injected) is the policy and the worker applies it. **Local wins when idle:** a flow started at the station preempts a parked, unseated hosting session (`yield_for_local`); a seated one is a held session. SO-101 only in this release. `GET /api/v1/hosting` publishes the wire contract (room, codec, fps, motors, cameras, `joint_ranges_deg`, `phase`, `active_operator`, `station_mode`); `/health` advertises `capabilities.hosting` while live.
