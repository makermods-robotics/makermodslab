...
`lerobot` is pinned by **SHA** to `Maker-Mods/lerobot@af9072a8` (branch `robot/makermods-maker-arm`), a clean fast-forward from upstream `v0.6.0` — the merge-base IS `v0.6.0` — carrying upstream `v0.6.2` plus six Maker-arm commits (see [pyproject.toml](pyproject.toml)). Pin by SHA, never by branch name: that branch is force-pushable and has already been renamed once (`arm/` → `robot/`). Track releases or fixed SHAs, never a moving branch — a moving pin makes every unrelated failure a bisect. lerobot is the one dependency the app cannot run without, so treat a bump as a real change: expect import-path drift, adjust call sites, and exercise a rollout end-to-end before landing it. The extras are `core_scripts,feetech,training` (SO-101) plus `maker` (RobStride over CAN) and `rebot` (motorbridge, the Star Arm 102's UART servo stack).
...
**Arm types — the Maker arm:**

Every robot record carries an `arm_type`: `so101` (the default, and what every record written before the Maker arm existed reads back as) or `maker`. The two share no bus protocol, no calibration procedure and no port-detection method, so it is the discriminant every hardware path branches on.

- [arm_capabilities.py](makermodslab/arm_capabilities.py) — **the one place that answers "what can this arm type do?"**. `uses_feetech_bus` gates every helper that reads or writes a servo register by name (`arm_identity`, `motor_power`, `rest_pose`, `identify`/`wiggle` — all Feetech-only, all skipped for a Maker arm); `joints_per_arm` is 6 vs **7**; `supports_auto_calibration` / `uses_zero_calibration` pick the calibration flow; `supports_dagger` is False for the Maker arm and it is a HARDWARE limit — the Star Arm 102 leader's joints hold encoders and no motors, so there is nothing to back-drive during a handover. Import this instead of writing `arm_type == "maker"` inline.
- [zero_calibrate.py](makermodslab/zero_calibrate.py) — the Maker arm's ONLY calibration: torque off, the user poses the arm by hand, we set zero, and the ranges come from fixed config constants. No range sweep (its joint limits are measured constants) and no auto-calibration. Deliberately NOT a new session kind — it reuses kind `calibration`, and `calibrate.calibration_is_active()` ORs both managers so every existing reciprocal mutex check covers it with no new `robot.busy.*` discriminant.
- [maker_ports.py](makermodslab/maker_ports.py) — port detection. The follower (RobStride/CAN) and leader (FashionStar/UART) answer _different protocols_, so `probe_maker_ports` identifies both with no user gesture; `identify_maker_arm_by_motion` is the fallback for a bimanual rig, whose two arms per side are indistinguishable by probe. Both are strictly read-only.

Calibration libraries are **per arm type** and never merged: `so_leader`/`so_follower` for the SO-101, `rebot_102_leader`/`maker_follower` for the Maker pair. lerobot derives those directory names from the device class's `name`, so they must match it exactly. Thread `arm_type` through `calibration_dir_for_device`, `setup_calibration_files`, the bimanual stagers, and `is_robot_record_clean` — a hardcoded SO-101 path makes a fully set-up Maker robot look permanently unready.

Two deliberate gaps: **no Maker URDF ships**, so a Maker teleop session broadcasts `joints_deg` (degrees by motor name) instead of URDF joints and the frontend renders `JointAngleReadout` in the 3D viewer's slot; and **episode replay's ease-in differs by arm type**. The playback loop itself is arm-agnostic for both (plain `send_action` on the dataset's action column, as lerobot's own `lerobot-replay` does it); what differs is getting to frame 0 and back. The SO-101 uses `return_to_rest_pose` with a raw-tick profile velocity; a Maker arm uses `_ease_in_maker`, which resends frame 0 and lets `startup_sync_speed_deg` cap the approach to a degree per step. That ease-in judges arrival by **convergence, not tolerance**: a Maker joint is position-controlled with MIT gains, so it holds a standing error its gains cannot close (measured 3-5 deg on `wrist_flex` across runs). Waiting for zero fails every time on a healthy arm — so once the worst joint stops improving, being within `_MAKER_EASE_SETTLE_DEG` counts as arrived, and only a joint that converges FAR from target is a real failure.
...
Two arm families, each single or **bimanual**:
...
- **Maker Arm v1** — a **7-DOF** RobStride follower on classic CAN (via an slcan adapter; `port` is the adapter's serial device, e.g. `/dev/cu.usbmodem*` on macOS) driven by a **Star Arm 102** (reBot 102) leader on FashionStar UART servos (`MakerFollowerConfig`/`RebotArm102LeaderMakerTeleopConfig`, `BiMakerFollowerConfig`/`BiRebot102LeaderMakerConfig` when bimanual). The leader MUST use the `_maker` preset, not the bare `rebot_102_leader`: the base config describes the reBot B601 follower, and pointed at a Maker arm unchanged most joints run the wrong way or saturate against the follower's soft limits while teleop keeps reporting a healthy loop.
