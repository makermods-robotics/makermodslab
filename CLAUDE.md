...
- [remote_inference.py](makermodslab/remote_inference.py) — remote inference (DRTC): the arm is driven locally while the policy runs on a remote GPU, the two meeting over a LiveKit room. This process owns **only** the robot side; it verifies the SFU and that an operator is present before energizing, and never launches the GPU. [drtc_protocol.py](makermodslab/drtc_protocol.py) is the stdin/stdout line protocol between it and the child, its own module for the same reason `eval_protocol.py` is — the parent must never import `livekit.portal` (an FFI dylib behind the optional `[drtc]` extra) and the child is exactly the process that does. `drtc/` holds the child entrypoints and the LiveKit plumbing; nothing there is imported at server boot. Only three v1 routes are its own (`/remote-inference-status`, `/remote-inference/transport`, `/remote-inference/clear-local-override`) — start and stop ride the sessions surface as kind `remote_inference`.
