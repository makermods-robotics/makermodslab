...
- [remote_host.py](makermodslab/remote_host.py) — session kind `hosting`, the STATION side: "Available for remote teleop". Opens the record's follower(s) WITH the record's cameras, joins this node's room as Portal's `Robot` through the `lerobot-teleoperator-livekit` plugin, publishes state + frames and executes the room's ACTIVE operator's actions. Never opens a leader; deliberately operator-agnostic (a laptop leader today, a policy worker in the remote-inference phase, both against this unchanged session). The loop is teleoperate.py's loop with the network as the teleop device; safety (start-pose capture, return-to-rest on stop, explicit release, two-press stop) is teleoperate.py's, reused. One knob, `fps`, paces frames, state and actions. `GET /api/v1/hosting` publishes the wire contract (room, codec, fps, motors, cameras, and the SO-101 follower's calibrated `joint_ranges_deg` so the OPERATOR can render the URDF without the station's calibration file); `/health` advertises `capabilities.hosting` while live.
- [remote_teleoperate.py](makermodslab/remote_teleoperate.py) — session kind `remote_teleoperation`, the OPERATOR side: this node's leader drives a station's follower. The station must already be hosting; this side starts nothing there. Handshake through the node registry (resolve → hosting descriptor → arm-family/motor-set check → operator token from the station's `/sfu/token`), then the leader is wrapped by `lerobot-robot-livekit`'s `LiveKitRobot` and the local teleop loop runs unchanged. **The plugin names are inverted relative to where they run:** `LiveKitTeleoperator` runs ROBOT-side (it IS the teleoperator from the follower's point of view), `LiveKitRobot` runs OPERATOR-side. The remote follower's joints ride the normal WS broadcast; remote cameras are re-streamed as MJPEG on `/api/v1/remote-teleoperation/camera/{name}` (one extra hop; a browser-joins-the-room phase can replace it). Readiness uses the new `arms="leader"` scope — a laptop record with no follower is the expected shape.
