...
- **`uvicorn --reload` restarts.** Both this investigation's two runs used a real `kill -TERM` against a **prod-mode** process (`_run_prod()`, no `--reload`). `--reload` goes through uvicorn's separate file-watcher/reloader supervisor process, which is a different signal-delivery path (parent forwards the signal to a child worker, on its own timing) — not exercised here at all, despite being named as one of the two triggers this whole investigation (and I8 itself) is meant to cover. **Related real-world observation (2026-08-07, unrelated to teleop):** the `--reload` shutdown path WAS seen hanging in the real world — but the cause was camera preview threads, not the teleop/recording stop path. A camera replugged into the same USB port after worker start keeps its AVFoundation uniqueID, so the preview opened a stale device whose first `cap.read()` blocked forever while holding `_SharedCapture.lock` in `makermodslab/camera_preview.py`; graceful shutdown then never completed and the old worker needed SIGKILL. `camera_preview.py` has since been hardened (bounded lock acquisition + first-frame deadline; wedged captures are leaked, loudly, instead of hanging shutdown). If a `--reload` restart hangs again, rule out wedged camera threads before suspecting I8's teleop stop wiring.
