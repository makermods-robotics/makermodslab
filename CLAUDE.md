...
Two deliberate gaps: **no Maker URDF ships**, so a Maker teleop session broadcasts `joints_deg` (degrees by motor name) instead of URDF joints and the frontend renders `JointAngleReadout` in the 3D viewer's slot; and **episode replay's approach/return mechanism differs by arm type**. The playback loop itself is arm-agnostic for both (plain `send_action` on the dataset's action column, as lerobot's own `lerobot-replay` does it); what differs is getting to frame 0 and back, which is `rest_pose.py` for an SO-101 and `maker_rest_pose.py` for a Maker arm.
