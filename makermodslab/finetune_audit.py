...
---------------------------------------
MakerMods Lab launches a fine-tune as ``--policy.type <A> --policy.pretrained_path
...
2. **The run log** — lerobot's ``policies/utils.log_model_loading_keys`` emits
   ...
   MakerMods Lab captures the trainer's stderr into ``log.jsonl``. This is
...
PROVEN = "proven"
#: Consistent with the artifacts but resting on MakerMods Lab's own bookkeeping,
...
def audit_train_config(
    ...
    if train_config.get("resume") is True:
        ...
        # one source, so they cannot disagree by construction. MakerMods Lab's
...
def audit_load_warnings(messages: Iterable[str]) -> LoadWarnings:
    ...
    `messages` is the sequence of log lines (for MakerMods Lab that is the
