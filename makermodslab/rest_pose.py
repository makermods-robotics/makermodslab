...
def _return_one_bus_to_rest(bus, pose: dict, abort_event: threading.Event) -> None:
    """Drive one follower bus back to its captured pose; log start and outcome.

    The per-arm body of return_buses_to_rest, run on its own thread so
...
def return_buses_to_rest(rest_poses: list[tuple], abort_event: threading.Event) -> None:
    ...
    threads = [
        threading.Thread(
            target=_return_one_bus_to_rest,
