...
CLI knobs:
  ...
  --flow-steps                override the sampler's steps per chunk (opt-in)
  --extra-image-roles         declare extra camera views on the checkpoint (opt-in)
...
def describe_device(device: torch.device) -> str:
    ...
    Hand-mirrored from `policy.py`; change one, change the other.
    """
    if device.type != "cuda":
        return str(device)
...
def load_policy(
    ...
    * **``--extra-image-roles``, when passed, DECLARES extra camera views on
      ...
      the feature is genuinely enough for the one family that is not, and why
      the processors need an override of their own.
