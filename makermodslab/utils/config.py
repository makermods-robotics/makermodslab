...
def load_robot_cameras(robot_name: str, *, resolve_identity: bool = True) -> dict[str, dict]:
    ...
    (`reanchor_camera_indices`). Inference passes resolve_identity=False
    here and re-anchors only its selected bindings, so unused saved cameras
    need not be attached. Teleoperation and replay open no cameras at
    ...
    return reanchor_camera_indices(cameras) if resolve_identity else cameras
...
def bind_robot_cameras(
    ...
    # Only bound cameras participate in inference; an unplugged unused camera
    # must not prevent a policy from starting. Resolve after selecting below.
    available = load_robot_cameras(robot_name, resolve_identity=False)
    selected_names = {str(name or "").strip() for name in bindings.values()}
    reanchor_camera_indices({name: entry for name, entry in available.items() if name in selected_names})
