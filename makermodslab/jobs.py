...
class JobRecord(BaseModel):
    ...
    # Zero-padded step dirs known to be present under checkpoints/ in that repo.
...
def missing_pretrained_files(names):
    ...
    paths inside one checkpoints/<step>/). The fine-tune half of
    missing_checkpoint_files: a fine-tune reads pretrained_model/ only, so
    training_state/ is not required — and neither is train_config.json, which a
    Hub-imported base (a flat model repo laid out by push_to_hub, not by a
    checkpoint save) legitimately lacks. config.json is what the HF Jobs
    wrapper itself gates the pod-side download on; the weights are matched by
    suffix for the same reason as the resume rule (PEFT adapters rename them).
    """
    missing = []
    if "pretrained_model/config.json" not in names:
        missing.append("pretrained_model/config.json")
...
class JobRegistry:
    ...
    def start(self, config: TrainingRequest, target: JobTarget | None = None) -> JobRecord:
        ...
        # siblings inside it, deliberately: the two _check_pretrained_* calls
        # must run against the LOCAL ABSOLUTE PATH (they read the checkpoint's
        # config.json off this disk), so the rewrite can only happen once they
        # have. The rewrite lands on `config` itself, which is what gets
        # persisted on the record and what the runner reads — so the history
        # shows the ref that actually ran, not a host path that never could
        # (and the runner's own host-path refusal becomes belt-and-braces, for
        # requests that bypass the registry entirely).
    ...
    def _read_job_counter(self) -> int:
        ...
        unparseable JSON, wrong type, a value below 1. 0 means "no opinion", and
