...
import sockThumb from "@/assets/policy-sock-orange.jpg";
import bottleThumb from "@/assets/policy-bottle.jpg";
import towelThumb from "@/assets/policy-towel.jpg";
import stackCubesThumb from "@/assets/policy-stack-cubes.jpg";

/** Synthetic ids for policy previews that haven't been trained yet — no Hub
 * repo or job backs any of these, so none ever comes from `/models`. See WIP
 * handling in PolicySlider/PolicyDetailDialog. */
export const WIP_POLICY_IDS = {
  bottleCap: "wip/bottle-cap-removal",
  towelFold: "wip/towel-fold",
  stackCubes: "wip/stack-cubes",
} as const;

const WIP_POLICY_ID_SET: ReadonlySet<string> = new Set(
  Object.values(WIP_POLICY_IDS),
);

/** True when `id` is one of the not-yet-trained WIP preview cards. */
export function isWipPolicyId(id: string): boolean {
  return WIP_POLICY_ID_SET.has(id);
}

/** Curated preview media for featured policies, keyed by Hub repo id. */
const POLICY_THUMBNAILS: Record<string, string> = {
  "makermods/act_makermods_sock_2_only_more_orange_2026-07-16_22-14-55":
    sockThumb,
  [WIP_POLICY_IDS.bottleCap]: bottleThumb,
  [WIP_POLICY_IDS.towelFold]: towelThumb,
  [WIP_POLICY_IDS.stackCubes]: stackCubesThumb,
};

/** The curated preview image for a policy, or undefined when it has none. */
export function policyThumbnail(m: ModelItem): string | undefined {
  return POLICY_THUMBNAILS[m.hf_repo_id ?? m.id];
}

/** Curated display names for featured policies, keyed by Hub repo id. The repo
 * ids are DATA — they address a real repo, so they stay verbatim; only the
 * slug they map to is a translation key. */
const POLICY_NAME_KEYS: Record<string, string> = {
  "makermods/act_makermods_sock_2_only_more_orange_2026-07-16_22-14-55":
    "sortingSocks",
  [WIP_POLICY_IDS.bottleCap]: "openingBottleCaps",
  [WIP_POLICY_IDS.towelFold]: "foldingTowels",
  [WIP_POLICY_IDS.stackCubes]: "stackingCubes",
};

/** The marketplace provenance of a policy card. "wip" marks a preview card for
 * a policy that hasn't been trained yet — no repo/job backs it. */
export type PolicyBadge = "mine" | "makermods" | "community" | "wip";
...
/** The Hub namespace a policy lives under, or null for a bare local run id (no
 * "/"), which belongs to the logged-in user by construction. */
export function policyNamespace(m: ModelItem): string | null {
  const src = m.hf_repo_id ?? m.id;
  return src.includes("/") ? src.split("/")[0] : null;
}

/** The byline shown under a policy's title. A WIP preview has no author — its id
 * is synthetic, so the "wip/" prefix is a keying device and not an org, and
 * `policyNamespace` would otherwise surface it as one. */
export function policyAuthorLabel(m: ModelItem): string {
  if (isWipPolicyId(m.id)) return "Coming soon";
  return policyNamespace(m) ?? "local checkpoint";
}

/** Translated author byline. Mirrors policyTitle/policyDisplayTitle: the plain
...
export function policyDisplayAuthorLabel(t: TFunction, m: ModelItem): string {
  if (isWipPolicyId(m.id)) return t("launchpad.policies.comingSoon");
  return policyNamespace(m) ?? t("launchpad.policies.localCheckpoint");
}

/** MINE when the policy is in the user's namespace (or a bare local run they own);
...
/** True when the policy belongs in the user's library: any local checkpoint
 * (local/both) or a Hub repo in their own namespace. */
export function isMinePolicy(m: ModelItem, username?: string | null): boolean {
  if (m.source === "local" || m.source === "both") return true;
  return classifyPolicy(m, username) === "mine";
}

/** The UNTRANSLATED title: a curated English display name when the policy has
 ...
 * which language the UI is in — see PolicySlider's filter. Use
 * `policyDisplayTitle` for anything the user actually reads.
 */
export function policyTitle(m: ModelItem): string {
  const slug = POLICY_NAME_KEYS[m.hf_repo_id ?? m.id];
  if (slug) return POLICY_NAME_EN[slug];
  const raw = m.name || m.id;
  return raw.includes("/") ? (raw.split("/").pop() ?? raw) : raw;
}

/** English copies of the curated names, so `policyTitle` stays a pure data
...
/** The title the user sees. Falls back to the raw name segment for any policy
...
export function policyDisplayTitle(t: TFunction, m: ModelItem): string {
  const slug = POLICY_NAME_KEYS[m.hf_repo_id ?? m.id];
  if (slug) return t(`launchpad.policyNames.${slug}` as never);
  const raw = m.name || m.id;
  return raw.includes("/") ? (raw.split("/").pop() ?? raw) : raw;
}

const BADGE_LABEL_KEY: Record<PolicyBadge, string> = {
...
const BADGE_CLASS: Record<PolicyBadge, string> = {
 ...
 * themes. */
export const PolicyBadgePill: React.FC<{ badge: PolicyBadge }> = ({ badge }) => {
...
/**
 * One policy in the launchpad slider. Preview media slot + title + author +
 ...
 * Click opens the policy detail dialog.
 */
const PolicyCard: React.FC<PolicyCardProps> = ({ model, badge, onOpen }) => {
  const { t } = useTranslation();
  ...
  const policy = model.policy_type
    ...
    : null;
  const thumbnail = policyThumbnail(model);
  ...
  return (
    <button
      ...
      aria-label={t("launchpad.policies.open", { title })}
    >
      {thumbnail ? (
        <img
          src={thumbnail}
          alt={t("launchpad.policies.previewAlt", { title })}
      ...
      <div className="flex flex-1 flex-col gap-2 p-3">
        ...
        <div className="mt-auto flex flex-wrap gap-1.5 pt-1">
          ...
          {model.steps != null && (
            <Stat>
              {t("launchpad.policies.steps", {
                steps: formatCount(model.steps),
              })}
            </Stat>
          )}
          {model.private && (
            <Stat tone="amber">{t("launchpad.policies.private")}</Stat>
          )}
        </div>
      </div>
    </button>
