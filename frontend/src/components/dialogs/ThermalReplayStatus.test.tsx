...
afterEach(cleanup);
const status: Status = {
  result: "overheated", message: "shoulder_lift reached 130°C", experiment: "baseline",
  elapsed_s: 10, duration_s: 1800, cycles: 1, critical: false, rest_reached: true, log_dir: "/run",
  actuators: [{ actuator: "follower.shoulder_lift", temperature_c: 130, torque_nm: 6.2, status: "OVERHEATING" }],
};
it("shows the thermal alert and temperature and torque together", () => {
  render(<ThermalReplayStatus status={status} />);
  expect(screen.getByRole("alert")).toHaveTextContent("130°C");
