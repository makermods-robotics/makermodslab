...
const targets = { targets: null, profile: "", environment: "", setProfile: vi.fn(), setEnvironment: vi.fn() };
const config = { ...DEFAULT_REMOTE_RUN_CONFIG, policyHubId: "someone/p" };

function Harness({ state = "idle", pending = false, restarting = false, task = "pick up the block" }: { state?: GpuState; pending?: boolean; restarting?: boolean; task?: string }) {
  ...
  const status = state === "idle" ? null : {
    ...
    slack: 5, elapsed_s: 0, idle_stop_in_s: null,
  } as GpuStatus;
  return <>
    <GpuLaunchSection
      launcher={{ status, pending, restarting, error: null, start, stop, restart, refresh: vi.fn(), launched: null }}
      targets={targets} knobs={knobs} knobSupport={support} config={config}
...
describe("sync slack", () => {
  it("updates launch and copied command together and remembers the selection", () => {
    ...
    const select = screen.getByLabelText("Sync slack (ticks)");
    ...
    view.unmount();
    render(<Harness />);
    expect(screen.getByLabelText("Sync slack (ticks)")).toHaveTextContent("2");
  });

  it.each(["starting", "ready", "stopping"] as const)("is locked while the GPU is %s", (state) => {
    render(<Harness state={state} />);
    expect(screen.getByLabelText("Sync slack (ticks)")).toBeDisabled();
  });
...
describe("GPU restart", () => {
  it("wires the restart button to the selected settings and keeps Stop separate", () => {
    localStorage.setItem("makermodslab.gpuSyncSlack", "2");
    render(<Harness state="ready" />);
    fireEvent.click(screen.getByRole("button", { name: "Restart GPU with these settings" }));
  ...
  it("shows pending restart even after the old GPU reaches idle", () => {
    ...
    expect(screen.getByRole("status")).toHaveTextContent("requesting a replacement");
    expect(screen.getByLabelText("Sync slack (ticks)")).toBeDisabled();
  });

  it("does not stop a GPU when the replacement is missing a required task", () => {
    render(<Harness state="ready" task="" />);
    expect(screen.getByRole("button", { name: "Restart GPU with these settings" })).toBeDisabled();
  });
