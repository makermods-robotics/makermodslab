...
/**
 * The transport knobs, behind Advanced.
 *
 * They are not defaults anyone should tune casually — horizon, fps and codec
 * MUST match the GPU side, and Portal fingerprints the wire schema, so a
 * disagreement silently drops every packet instead of raising. That is exactly
 * why they are collapsed with their values on the summary line: the numbers
 * stay readable at a glance without inviting a fiddle.
 *
...
const RemoteAdvancedSection: React.FC<{
  ...
  disabled?: boolean;
}> = ({ config, onChange, checkpointHorizon, open, onOpenChange, disabled }) => {
  ...
  // Every value on the summary line is live and is DATA — the codec id is the
