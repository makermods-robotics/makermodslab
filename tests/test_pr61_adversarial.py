...
def test_delete_from_the_clobbered_row_destroys_unpublished_checkpoints(
    ...
    (dl / "checkpoints" / "100" / "pretrained_model").mkdir(parents=True)
    (dl / "checkpoints" / "100" / "pretrained_model" / "config.json").write_text(
        json.dumps({"type": "act"})
    )
    ...
    with patch("makermodslab.models.rollout") if False else patch(
        "makermodslab.models._model_in_use", return_value=None
    ):
