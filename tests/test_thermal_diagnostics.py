...
from makermodslab.thermal_diagnostics import ThermalDiagnostics, decode_feedback
...
def test_raw_capture_fault_latch_and_no_automatic_fault_clear(tmp_path):
    sent = []
    ...
    assert not sent
    diag.snapshot()
    assert "a3e8" in (tmp_path / "can_frames.jsonl").read_text()
    diag.close()
