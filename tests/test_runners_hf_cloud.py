...
derivation and the cloud-boundary config localization, plus the terminal-stage
bookkeeping in HfCloudJobRunner.stop() (which decides whether a stopped run
reads as `interrupted` or as a failure, and needs only a two-method fake).
Submission, log tailing and status polling talk to HF Jobs and are
intentionally left for integration tests."""
