...
def test_invalid_gripper_limit_leaves_no_record_or_directory(tmp_path, monkeypatch, value):
    directory = tmp_path / "not-created"
    monkeypatch.setattr(cfg, "ROBOTS_PATH", str(directory))
    with pytest.raises(ValueError, match="finite positive"):
        cfg.save_robot_record("grip", {"gripper_closing_error_deg": value})
    assert not directory.exists()


def test_gripper_limit_patch_preserves_and_explicit_null_disables():
    cfg.save_robot_record("grip", {"arm_type": "metal", "gripper_closing_error_deg": 2.5})
    cfg.save_robot_record("grip", {"follower_port": "/dev/example"})
    assert cfg.get_robot_record("grip")["gripper_closing_error_deg"] == 2.5
    cfg.save_robot_record("grip", {"gripper_closing_error_deg": None})
    assert cfg.get_robot_record("grip")["gripper_closing_error_deg"] is None


def test_gripper_limit_cleared_on_arm_family_change():
    cfg.save_robot_record("grip", {"arm_type": "metal", "gripper_closing_error_deg": 2.5})
    cfg.save_robot_record("grip", {"arm_type": "so101"})
    assert cfg.get_robot_record("grip")["gripper_closing_error_deg"] is None


def test_gripper_limit_reaches_both_local_session_request_builders():
    ...
    assert _build_teleoperation_request(record, TeleoperationOptions()).gripper_closing_error_deg == 2.5
    options = RecordingOptions(dataset_repo_id="local/test", single_task="grasp")
    assert _build_recording_request(record, options).gripper_closing_error_deg == 2.5


def test_gripper_limit_api_rejects_invalid_patch_atomically():
    from makermodslab.server import upsert_robot

    cfg.save_robot_record("grip", {"arm_type": "metal", "gripper_closing_error_deg": 2.5})
    response = upsert_robot("grip", {"gripper_closing_error_deg": -1, "follower_port": "/dev/new"})
    assert response.status_code == 400
    record = cfg.get_robot_record("grip")
    assert record["gripper_closing_error_deg"] == 2.5
...
@pytest.mark.parametrize(
    "body",
    [
        {"gripper_closing_error_deg": 2},
        {"arm_type": "so101", "gripper_closing_error_deg": 2},
    ],
)
def test_gripper_limit_api_refuses_unsupported_family(body):
    from makermodslab.server import upsert_robot
...
def test_gripper_limit_unknown_arm_keeps_coded_refusal():
