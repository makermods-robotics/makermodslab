...
def test_send_cap_leaves_control_ticks_available_and_does_not_catch_up():
    ...
    assert sends == [0, 0.2, 0.4, 0.6, 0.8]
...
@pytest.mark.parametrize(
    ...
    [
        {"camera_send_hz": -1},
        {"camera_send_hz": 21},
        {"camera_send_hz": float("nan")},
        {"engine": "sync", "camera_send_hz": 5},
        {"video_quality": 0},
        {"video_quality": 101},
        {"video_quality": 50.5},
        {"video_bitrate_kbps": 255},
        {"video_bitrate_kbps": 20001},
        {"latency_k": -1},
        {"latency_k": float("inf")},
    ],
)
def test_invalid_network_options(options):
    with pytest.raises(ValidationError):
        RemoteInferenceOptions.model_validate(
            {"policy_ref": "hub:someone/p", "engine": "rtc", "fps": 20, **options}
        )
