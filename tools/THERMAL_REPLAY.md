...
simulated feedback; physical return and gain behavior still require a supervised trial.
...
2. Stop recording. Put the follower at that supported rest pose, within 5 degrees
   ...
   destination. A pose is not automatically safe merely because it is called rest.
3. Open that episode's **Replay on hardware** panel. Select **Repeat as a 5-minute
...
4. Start and supervise. The UI shows each motor's temperature, signed torque, and
   status. Above 65°C is red; above 70°C is critical. A persistent message and toast
...
- Any fresh actuator temperature **>65°C** ends repeated motion and requests a
  controlled return to the captured rest pose at up to 20 degrees/second. Thermal
  readings continue during return; crossing **>70°C** also raises the critical flag.
- A run completes after 300 seconds of repeated playback, excluding initial
  alignment and final return. A bounded alignment between loops bridges any
  small endpoint mismatch and counts toward those 300 seconds. Arrival must be measured within 2 degrees before
...
- Missing/invalid feedback, feedback older than 250 ms, >12° tracking error for
  > 0.5 seconds, target clipping, or >100 ms playback lateness invalidates the run
...
API durations can be 10–600 seconds for supervised commissioning. The UI uses 300.
