...
>   IDs are carried on each heading; new findings used **N1–N11** — *renumbered
>   2026-08-02 to **T12–T22**; see the note at the top of this file.*
...
- `TeleopDialog.tsx:13-19` says the session state machine is "ported **verbatim** from pages/Teleoperation.tsx". It is — including every defect. **T1, T8, T9, T11 now exist twice.** Any fix must land in both files, or the dead page must be deleted (see T16).
...
> latch. **Adopting `useSessionExitGuard` (see T12) closes the page-leave half of this in one change.**
...
**First identified:** 2026-07-14
**Fix:** [PR #6](https://github.com/makermods-robotics/makermodslab/pull/6) (closed) · branch `fix/t3-orphaned-worker-thread` · [PR #21](https://github.com/makermods-robotics/makermodslab/pull/21) (merged) · branch `fix/t3-orphaned-worker-thread`
...
> global writes at `:778-790`, not just the null-ing. T18 is the mechanism that makes the "still alive after
> 5 s" state reachable.
...
> *before* the arm is driven home, so inference is admitted during the entire releasing window (see T14 for the
...
> **New strengthening (T15):** the teleop window is a plain `fixed` div with **no backdrop, no focus trap**
...
**First identified:** 2026-07-14
**Fix:** [PR #5](https://github.com/makermods-robotics/makermodslab/pull/5) (closed) · branch `fix/t7-startup-torque-warning` · [PR #20](https://github.com/makermods-robotics/makermodslab/pull/20) (merged) · branch `fix/t7-startup-torque-warning`
...
> worker's `finally` (`:770-771`), which a failed start never reaches. See T22: this leaves zero persistent
...
> same defect as T13 ("the UI disappears while the arm is still energized and moving") — hence raised to P1.
...
### T12 · P1 — Teleoperation is the only live-hardware surface with no page-leave guard *(formerly N1)*
...
**Impact:** the operator loses the one confirmation the other three hardware surfaces give them, and the only stop that fires is the unacknowledged keepalive of T1. **Fix note:** adopting `useSessionExitGuard` here closes T12 and the page-leave half of T1 in one change.

### T13 · P1 — Done closes the only session window while the arm is still energized and moving *(formerly N2)*
...
### T14 · P1 — `finish_pending_release()`'s failure return is ignored by both start paths *(formerly N3)*
...
### T15 · P1 — The live teleop window is non-modal: the rest of the app stays interactive during a session *(formerly N4)*
...
### T16 · P2 — The teleop state machine is duplicated, and `/teleoperation` is a live footgun *(formerly N5)*
...
### T17 · P2 — The 13 s post-stop cleanup check reads process-global state a newer session clears *(formerly N6)*
...
### T18 · P2 — The rest-return join is unbounded, and part of the return sits outside both the ceiling and the abort *(formerly N7)*
...
### T19 · P2 — The stop path mutates shared state without `_state_lock` *(formerly N8)*
...
### T20 · P2 — The WebSocket broadcast queue is unbounded and its overflow branch is dead *(formerly N9)*
...
### T21 · P2 — `strict=False` hides a telemetry/bus count mismatch *(formerly N10)*
...
### T22 · P2 — A failed start leaves no trace in the status payload *(formerly N11)*
...
6. **PR overlap:** #5/#6/#7 target T7/T3/T4. Each was verified against `main`'s code independently. T14 and T15 sit inside T4's family and T18 inside T3's — a PR that closes only the entry as originally written will leave those open.
