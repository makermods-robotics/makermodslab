...
*Renumbered 2026-08-02 into one continuous per-module sequence: the two original entries became **D1**/**D2**, and the former appended `N`-series became **D3–D26** (N1→D3 … N24→D26). Every bug entry now carries a **First identified** date (and **Resolved**, where the entry attests a fix or a descope). Entries may also carry a **Fix:** line, directly under **First identified**, linking the pull request(s) that fix or attempt to fix them.*
...
> - **PRs #5–#11 are CLOSED with `merged=no`.** PR #8 (Dataset D1 visibility) targeted this backlog and did
...
### D1 · In progress — Local dataset cloud-training notice says private, but upload is public
...
sibling D3 / Recording R17). So this is no longer *silent publication of private data* — it is a **copy bug**:
...
> separately (see **D3**, a fourth, worse path). *Cannot verify statically:* the live repo's actual
...
### D2 · Open — Dataset in-use guard does not cover training jobs
...
>   (`datasets.py:731-733`) but **not the merge sources** — see **D8**.
...
### D3 · ~~P0~~ — ~~Recording auto-pushes the just-recorded dataset to the Hub as PUBLIC~~ — **DESCOPED, not a bug** *(formerly N1)*

**First identified:** 2026-07-23 · **Resolved:** 2026-07-26 — descoped (product decision)

> Was **CURRENT P0-2**, found independently as **Recording R17**. **Both IDs are retired, not reused.**
...
Knock-on for this module: **Dataset D1 survives as a copy bug** (the cloud notice promises *private* while every path publishes public — correct the notice, not the behavior) and **Dataset D12** as a centralization issue. See CURRENT → "Descoped by product decision".

### D4 · P1 — A failed/aborted Hub download can leave a **partial dataset that passes every "is it complete?" check** *(formerly N2)*
...
### D5 · P1 — Local dataset delete does **not** invalidate the Hub-status cache — and the two sibling discard paths do *(formerly N3)*
...
### D6 · P1 — On `main` there is **no UI at all** to delete a local-only dataset *(formerly N4)*
...
### D8 · P2 — `_dataset_in_use` does not protect merge **sources** *(formerly N6)*
...
### D9 · P2 — Merge completion never invalidates the dataset listing cache *(formerly N7)*
...
### D10 · P2 — A completed recording session never invalidates the dataset listing cache either *(formerly N8)*
...
### D11 · P2 — Logging in does not invalidate the dataset listing cache or the Hub-status cache *(formerly N9)*
...
### D12 · P2 — `get_hub_status`'s `on_hub` answer is cached for the process lifetime and is load-bearing for the cloud gate *(formerly N10)*

**First identified:** 2026-07-23

`datasets.py:213-217` memoizes `on_hub` forever. `jobs.py:1116-1120` uses it as the *only* gate preventing a cloud job on a local-only dataset. If the repo is deleted on the Hub (or moved) after being cached, the gate passes and control lands in `_ensure_dataset_on_hub`, which pushes the local copy **publicly** (`hf_cloud.py:671`). This is a second, cache-driven route into the D3/D1 visibility problem. Nothing outside MakerMods Lab invalidates the entry. *(Staleness window needs a running server plus an out-of-band Hub deletion to observe.)*

### D13 · P2 — `_ensure_dataset_on_hub` catches only `RepositoryNotFoundError` *(formerly N11)*
...
### D14 · P2 — The Hub-upload path "uploads" datasets it may first have to download *(formerly N12)*
...
### D15 · P2 — `UploadManager` / delete / rename share no lock — TOCTOU on the in-use guard *(formerly N13)*
...
### D16 · P2 — `_dataset_in_use` does a filesystem scan per registry record on every delete/rename/upload *(formerly N14)*
...
### D17 · P2 — `MergeManager.get_status` drains the log queue destructively *(formerly N15)*
...
### D18 · P2 — `useDatasets` wipes the list to empty on any fetch failure *(formerly N16)*
...
### D19 · P2 — Per-author Hub listing is silently truncated at 200 *(formerly N17)*
...
### D20 · P2 — Stale docstrings claim "private by default" where the code is public by default *(formerly N18)*
...
### D21 · P2 — Pinned custom Hub datasets always render as public in the library *(formerly N19)*
...
### D22 · P0 — A bare dataset name resolves to the same level as MakerMods Lab's own state dirs, and delete `rmtree`s whatever is there *(formerly N20)*
...
and the disk-import path defaults to the source folder's basename with no namespace at all (see **D23**).
...
### D23 · P1 — Neither import nor the logged-out record path prefills the Hub namespace, so both mint bare names *(formerly N21)*
...
1. Bare-named datasets, which are the precondition for **D22**.
...
So a logged-out / offline station (the documented deployment mode — see Config C10) records straight
into a bare-named directory. Cross-filed as **Recording R25**.

### D24 · P2 — `import_local_dataset` copies synchronously inside the request *(formerly N22)*
...
### D25 · P2 — `DatasetCard` truncates the one field users disambiguate by, then repeats it twice more *(formerly N23)*
...
### D26 · P2 — The Launchpad library sheet lists datasets unfiltered, unsearchable and uncapped *(formerly N24)*
...
1. **`frontend/src/components/landing/DatasetsPanel.tsx` does not exist** on `main`. The dataset surfaces are `DatasetPicker.tsx` (now unreferenced dead code — see D6), `DatasetInfoCard.tsx`, `DatasetLibrary.tsx` (`components/library/`), `ManageCachesDialog.tsx`, `MergeDatasetsDialog.tsx`, `UploadDatasetDialog.tsx`, `DatasetDetailDialog.tsx` (`components/dialogs/`) and `CollectHandoff.tsx` / `CollectPanel.tsx` (`components/studio/`). That set was audited instead.
2. **The `Training.tsx:456` upload call site in Dataset D1 no longer exists.** `Training.tsx` on `main` is a 105-line router wrapper; the upload chain moved to `TrainingConfigurator.tsx:429-432`. The defect survived the move — same `private=false`.
3. **Dataset D1's "In progress / Progress (2026-07-14)" section describes andrew-branch work that is not on `main`.** No visibility toggle in `LocalDatasetCloudNotice.tsx`, no `uploadMakePublic` state, no `private=True` in `hf_cloud.py`, no `tests/test_runners_hf_cloud.py` visibility assertion. On `main` the P0 is not fixed at all, and `hf_cloud.py:669-670` documents a deliberate move in the opposite direction.
4. Dataset D1's framing "notice promises private while both upload paths publish public" is accurate for `main`; the backend carries an explicit **public-by-default policy comment**. *(2026-07-26: the policy is now confirmed intended, so this is a pure copy divergence — the notice is the side that is wrong. The record-flow auto-push originally logged here as a fourth public path (D3) is **descoped**, not a defect.)*
