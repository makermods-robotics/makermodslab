...
*Renumbered 2026-08-02 into one continuous per-module sequence: the former appended `N`-series became **I13–I25** (N1→I13 … N13→I25); originals **I1–I12** and the design gaps **G1–G4** keep their IDs. Every bug entry now carries a **First identified** date (and **Resolved**, where the entry attests a fix or a descope). Entries may also carry a **Fix:** line, directly under **First identified**, linking the pull request(s) that fix or attempt to fix them.*
...
>   **N1–N10** — *renumbered 2026-08-02: the whole appended series became **I13–I25**;
...
**Fix:** [PR #9](https://github.com/makermods-robotics/makermodslab/pull/9) (closed) · branch `fix/i1-inference-startup-worker-no-stop-handle` · [PR #23](https://github.com/makermods-robotics/makermodslab/pull/23) (merged) · branch `fix/i1-inference-startup-worker-no-stop-handle` · [PR #27](https://github.com/makermods-robotics/makermodslab/pull/27) (draft) · branch `fix/i6-inference-startup-no-forced-recovery`
...
> `_inference_proc`; the P0-adjacent untracked-child variant is closed on that path (replaced by **I14**). **New,
...
> child exited. The only handle is discarded. Aggravates I13 (SIGKILL case still returns 200).
...
**First identified:** 2026-07-14
**Fix:** [PR #7](https://github.com/makermods-robotics/makermodslab/pull/7) (closed) · branch `fix/t4-cross-feature-mutex` · [PR #11](https://github.com/makermods-robotics/makermodslab/pull/11) (closed) · branch `fix/i4-calibration-atomic-exclusion-and-exit-guard` · [PR #22](https://github.com/makermods-robotics/makermodslab/pull/22) (open) · branch `fix/t4-cross-feature-mutex` · [PR #24](https://github.com/makermods-robotics/makermodslab/pull/24) (open) · branch `fix/i4-calibration-atomic-exclusion-and-exit-guard`
...
**First identified:** 2026-07-14
**Fix:** [PR #10](https://github.com/makermods-robotics/makermodslab/pull/10) (closed) · branch `fix/i5-shutdown-orphans-inference-subprocess` · [PR #25](https://github.com/makermods-robotics/makermodslab/pull/25) (open) · branch `fix/i5-shutdown-orphans-inference-subprocess`
...
> (`models.py:977-979`). See **I15** for a second, unguarded deletion route (`DELETE /jobs/{id}`) that makes this
...
### I13 · P0 — A Stop that overruns the 5 s budget SIGKILLs the rollout mid-teardown, leaving follower torque enabled; MakerMods Lab has no fallback release on any inference exit path *(formerly N1)*
...
### I14 · P0/P1 — The abandoned-after-spawn path can leak a live, completely untracked rollout subprocess *(formerly N2)*
...
### I15 · P1 — `DELETE /jobs/{job_id}` bypasses the inference in-use guard and `rmtree`s a checkpoint a live rollout is reading *(formerly N3)*
...
### I16 · P1 — During a status-endpoint outage the dialog shows no Stop control and the exit guard is disarmed *(formerly N4)*
...
### I17 · P2 — The subprocess is reaped only by a `/inference-status` poll; with nobody polling, the slot stays latched active forever *(formerly N5)*
...
### I18 · P2 — Bimanual right-arm fields are unvalidated, so a bad request opens and primes the LEFT arm before failing *(formerly N6)*
...
### I19 · P2 — Camera dict keys are interpolated into a draccus dict literal with no escaping *(formerly N7)*
...
### I20 · P2 — Camera bindings are frozen cv2 indices; a replug that reuses an index silently rebinds a role to a different physical camera *(formerly N8)*
...
### I21 · P3 — The final `proc.wait()` on the stop path has no timeout *(formerly N9)*
...
### I22 · P3 — Two independent Stop surfaces can double-signal the child, turning a clean user stop into a reported failure *(formerly N10)*
...
### I23 · P2 — `rollout.py` hardcodes the lerobot cache root twice, so inference logs ignore `HF_LEROBOT_HOME` *(formerly N11)*
...
### I26 · P2 — `friendly_hint` labels servo-bus failures as "Couldn't download the model" (fixed in working tree)

**First identified:** 2026-08-03

**Status:** Fix implemented same day as uncommitted working-tree edits (`utils/errors.py` + tests in `tests/test_rollout.py`); this entry records the defect for history.

lerobot's motors bus raises **every** serial failure as `ConnectionError` (`lerobot/motors/motors_bus.py:1117`), and `friendly_hint`'s Hub-network branch triggered on the bare token `"connectionerror"` with a self-satisfying second conjunct (the word "ConnectionError" contains "connect"). So a rollout subprocess dying at `robot.connect()` with `ConnectionError: Failed to write 'Lock' on id_=3 … [TxRxResult] Incorrect status packet!` was reported in the UI as "Couldn't download the model — check your internet connection" (observed live 2026-08-03, log `inference_logs/1785789729.log`). The file even carried a comment acknowledging the over-match, but only the camera case had been worked around. Not a phase-attribution bug: the hint is derived purely from error text (`rollout.py:1224, 2061, 2164, 2236` → `status.hint` rendered verbatim by `InferenceSessionDialog.tsx`).

**Fix (in tree):** a servo-bus branch ahead of the Hub branches keyed on motors-bus phrasing (`txrxresult`, `incorrect status packet`, `failed to (sync )read/write`), gated off when the text carries rollout's `"Failed to download the model: "` prefix; the Hub branch now requires a genuinely Hub-specific token (`huggingface.co` / `hf.co` / `max retries`) or that download-step prefix instead of bare `connectionerror`.

**Residual (not fixed):** `_classify_outcome`'s `CLEANUP_MARKERS` still matches `"overload"`/`"torque_enable"` anywhere in a 50-line log tail, so a run that merely logged an overload warning and then died of something else is downgraded to `ran_with_warning`; and the `"overload"` token ordering between `is_cleanup_error` and the new bus branch is now load-bearing.
...
- **Stop where the child needs >5 s to tear down: assert torque is released (or a fallback fires) rather than SIGKILL-and-forget (I13).**
- **Stop racing the spawn: assert no untracked process survives the abandon branch (I14).**
- **`DELETE /jobs/{id}` while an inference is running that job's checkpoint: assert 409 (I15).**
- **Status endpoint 5xx for N ticks while a run is live: assert a Stop control is still reachable and the guard armed (I16).**
- **Run ends by `--duration` with no status poller: assert the slot is released (I17).**
- **`duration_s = 0` submitted from the UI's number field: assert rejection (G2).**
- **Camera replug that reuses a cv2 index between picker-open and Start: assert the binding is invalidated (I20).**
...
4. **"arm-count validation before hardware is touched" holds; the general request-shape claim does not** — the bimanual right-arm fields are unvalidated (I18).
