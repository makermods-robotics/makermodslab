...
>   **#7 (T4, cross-feature mutex incl. calibration/auto-cal/wiggle)** overlaps entry 11 / N2, and
...
The original audit found no P0. **Re-verification adds one: N1 (P0)** — auto-calibration wipes the servos' persistent calibration registers before measuring, with no rollback on Stop/exception. This is **CURRENT P0-1**. See New findings below.
...
> — no register restore, no `warning` field. Compounds N1 (auto-cal has the same no-rollback hole on the
> primary path).
...
> See **N2** for the reachable consequence. PR #7 (`fix/t4-cross-feature-mutex`) adds calibration/auto-cal/
...
### N2 · P1 — Nothing prevents starting teleoperation, recording, or inference while arms are auto-calibrating under torque *(formerly NEW-2)*
...
### N3 · P1 — Calibration write-back bypasses the duplicate-port guard, and the resulting record still reports `is_clean: true` *(formerly NEW-3)*
...
### N4 · P1 — Camera identity binds to the browser `deviceId` via fuzzy name matching; the backend's stable `unique_id` is discarded *(formerly NEW-4)*
...
### N5 · P2 — `clamp_motor_power` raises on NaN/Infinity, contradicting its documented "never raises" contract, and can hide every robot from the UI *(formerly NEW-5)*
...
### N6 · P2 — Auto-calibrated leader runs leave a permanent scratch copy in `robots/so_leader/` *(formerly NEW-6)*
...
### N7 · P2 — `ports/{leader,follower}_port.txt` is dead persistence, and `/robot-port/{robot_type}` has no caller *(formerly NEW-7)*
...
### N8 · P2 — The `lelab_biso` → `makermodslab_biso` staging rename has no migration, unlike its two siblings *(formerly NEW-8)*
...
anything walking the cache — the same shape as **N6**'s scratch-copy accumulation.
...
### N9 · P2 — The Quality workflow has failed on `main` for days and gives no signal, because it lints the whole repo including vendored kernel source *(formerly NEW-9)*
