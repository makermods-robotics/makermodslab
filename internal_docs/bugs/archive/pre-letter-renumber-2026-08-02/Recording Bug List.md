...
>   automatic public Hub push was added** (new finding N2, ranked P0).
...
>   **N1–N9**. Nothing is renumbered. (Note: Recording's `D1–D5` design-gap prefix collides with Config/RANKING's
>   `D1–D3`; different modules, module prefix disambiguates.)
...
> **Verdict on `main` (2026-07-23): NOT APPLICABLE — SUPERSEDED (but see N4).** `main` never took the v0.6
...
> so recording is dead-on-arrival in this checkout as installed; see N4.** Per `CURRENT 23-07-26`, `main`'s
...
> **Verdict on `main` (2026-07-23): STILL REAL — and materially under-rated: it DOES destroy data (see N1).**
...
**Priority:** P2 — requires a direct API call or a poll racing a phase change; affects flag state, not data on disk. Backend phase validation should be authoritative (verified 2026-07-14 against the current working tree). **← Re-verification raises the real impact: see N1 (data loss).**
...
One consolidated, append-only list: every finding since the original audit gets the next `N` here, newest last. IDs are permanent and per-module — "Recording N1" is distinct from "Dataset N1" / "Inference N1". N1–N9 date from the 2026-07-23 re-verification, N10 from the 2026-07-24 working session; later entries carry **First identified** dates inline. *(Consolidated 2026-08-02 from the former "New findings" sections and later in-place appends; no prose, verdicts or severities were changed.)*
...
"Descoped by product decision" for the knock-on effects on Dataset entry 1 and Training N3 (formerly NEW-3).
...
### N4 · P1 (environment) — The installed environment contradicts `main`'s lerobot pin; recording fails immediately in this checkout as installed
...
### N10 · P2 — A logged-out Collect session records into a BARE dataset name, at the same level as MakerMods Lab's state dirs
...
An offline / logged-out station — the documented deployment mode, cf. Config entry 10 — therefore records into
...
2. The resulting bare-named dataset is the precondition for the destructive delete in **Dataset N20 (P0)** —
...
severity lives on the delete side in Dataset N20. Cross-filed as **Dataset N21**.

---

### N11 · P1 — Follower calibration writes are silently discarded: `connect()` leaves the EEPROM write-protected (`Lock=1`), then `_write_calibration` writes into it and logs success
...
case, and three of five robot flows plus both leader arms are structurally immune. Contrast Dataset N20, where
...
### N12 · P1 — One camera hiccup poisons every later recording in the process: the connect retry can't release a partially-connected robot, so the bus and camera read threads leak

**First identified:** 2026-07-29 — **fixed same day** (`record.py`, `teleoperate.force_disconnect_partial`)
...
### N13 · P1 — Chrome's camera hold outlives the preview UI **and makermodslab restarts**: record's connect races the browser's asynchronous device release, and after a camera replug it loses every time
...
After a camera unplug/replug (done to exercise the N12 fix), **every** recording start failed at
...
observation that separates this from N12-style in-process leaks.
...
**Related:** N6 (quitting re-opens browser previews while the backend owns the cameras — same ownership
conflict, opposite direction); R3/N12 (the failure-path leak that made earlier incarnations of this
unrecoverable in-process; with N12 fixed, tonight's failures stayed clean and retryable, which is what made the
...
### N14 · P1 — Identical-name cameras get paired to cv2 indices by coin flip: name-based browser matching swaps front/wrist silently, previews look correct while the dataset records crossed streams
...
### N15 · P2 — The preview-handover pattern wedges its feature flag (and now all previews) if `stop_all()` raises: shared shape across record, rollout, and focus-tune starts
...
- Ran `.venv/bin/python -m pytest tests/test_record.py -q`: **53 passed, 7 failed** (all the v0.6 `rgb_encoder` compatibility regression — see R1/N4).
...
1. **"recording frontend (`frontend/src/components/recording/`)"** contains only `RecordingSessionDialog.tsx` and `CameraConfiguration.tsx`. Request construction, exit handling and post-session handoff live in `frontend/src/components/studio/` (`CollectPanel.tsx`, `CollectHandoff.tsx`) and `StudioContext.tsx`. N2, N6, and R2's reach are only visible there.
