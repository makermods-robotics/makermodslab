...
> **2026-08-02 note:** this rollup is frozen as of 07-26. The per-module lists have since been restructured: all post-audit findings now live in one "Appended findings (N-series)" section per module, and Config/Training `NEW-n` IDs were renamed to `N<n>` (numbers unchanged). Read `NEW-n` below as `N<n>`.
