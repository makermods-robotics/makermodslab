...
These are additional entries surfaced by the re-audit; their IDs (N1–N19) are preserved as assigned. `N`-prefixed IDs are **per-module** — "Dataset N1" is distinct from "Recording N1" / "Inference N1".

### P0

#### N1. ~~Recording auto-pushes the just-recorded dataset to the Hub as PUBLIC~~ — **DESCOPED, not a bug**
...
Entries from a 2026-07-24 working session against `main` @ `ce42158`. IDs continue this module's
`N`-series (N20–N24); nothing above is renumbered. Same rubric as the 23-07-26 set. Analysis is again
**purely static** — no Hub call, no app run, no hardware; no code was modified.

### P0

#### N20. A bare dataset name resolves to the same level as MakerMods Lab's own state dirs, and delete `rmtree`s whatever is there
...
#### N21. Neither import nor the logged-out record path prefills the Hub namespace, so both mint bare names
...
#### N22. `import_local_dataset` copies synchronously inside the request
...
#### N23. `DatasetCard` truncates the one field users disambiguate by, then repeats it twice more
...
#### N24. The Launchpad library sheet lists datasets unfiltered, unsearchable and uncapped
