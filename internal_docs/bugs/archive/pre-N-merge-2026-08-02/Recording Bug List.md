...
IDs (N1–N9) preserved as assigned. `N`-prefixed IDs are per-module — "Recording N1" is distinct from "Inference N1" / "Dataset N1".
...
"Descoped by product decision" for the knock-on effects on Dataset entry 1 and Training NEW-3.
...
Entries from a 2026-07-24 working session against `main` @ `ce42158`. IDs continue this module's `N`-series;
nothing above is renumbered. Static analysis only — no hardware, no Hub call, no app run.
