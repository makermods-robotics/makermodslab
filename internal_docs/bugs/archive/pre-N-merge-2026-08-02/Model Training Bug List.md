...
>   **MT1–MT24**. New findings use **NEW-n** (NEW-1..NEW-10, per-module). Nothing renumbered.
...
> production observation below stands (nothing on `main` changed the mechanism). Related concurrency bug: NEW-9.
...
> NEW-1 + NEW-5** (runner finalization contract).
...
> **Verdict on `main` (2026-07-23): STILL REAL — and worse than recorded (see NEW-1).** `stop()` pre-sets
...
> **MakerMods Lab has no code path that can cancel the remote job** — see **NEW-1**. Correction to the original impact
> line ("no longer monitors"): `server.py:1364-1373` keeps a dismissed-but-active Hub job visible in `/jobs/hub`,
> so the gap is *actionability*, not visibility. **Fix cluster with MT10 + NEW-1 + NEW-5.**
...
> **Verdict on `main` (2026-07-23): STILL REAL (+ new UI symptom NEW-6).** Cloud resume reuses the parent's
...
> half — duplicate checkpoint entries with identical `step` in the lineage dropdown — is **NEW-6** (`JobCard.tsx:187-216`).
...
> `TrainingJobDialog.tsx:135-139`, `:154-164`, `:170-174`. Backend twin is NEW-4 (the persisted log is truncated
...
> (`:946-948`); the successful repo id is never written back to the source `JobRecord`. Related to NEW-8 (the
...
Ranked P0/P1/P2 per the rubric. IDs (NEW-1..NEW-10) preserved as assigned. `NEW-n` is per-module.
...
### NEW-3 · ~~P1~~ → P2 (narrow trigger) — Implicit cloud-run dataset upload decides visibility at its own site
...
### NEW-11 · P2 — `tests/test_models.py` reads the developer's real pin/hide files, so the model tests pass or fail depending on the machine
...
### NEW-12 · P2 — A cloud "Continue" silently drops the parent run's job timeout, falling back to 2h
...
**Severity:** P2 — wastes a paid run and its wall-clock, but publishes checkpoints normally along the way, so nothing is destroyed. Compare NEW-7 (seeded resume silently converted to a fresh full-length run), same class.
...
### NEW-13 · P1 — Cloud runs never set `save_checkpoint_to_hub`, so the full checkpoint is written to the container's disk and discarded; only the policy weights survive
...
- Training and upload controls do not consistently state that implicit dataset publication and model upload are public operations. *(See NEW-3.)*
- Custom model-upload metadata can omit the generic `lerobot` discovery tag. *(See NEW-8.)*
...
- **Entries 2, 3, 12 — Hub-checkpoint listing semantics.** One "list the root policy alongside the tree + disambiguate lineage/step refs" change addresses all three. Add **NEW-6** (the UI symptom of 12).
...
- **Entries 10, 11 — runner finalization contract.** A runner that cannot know (or was told to stop) reporting a definitive `returncode()` the watchdog converts into an irreversible terminal state. **Re-verification: absorb NEW-1 and NEW-5** — all four are the same contract violation.
...
2. **MT11's "no longer monitors" is imprecise.** `server.py:1364-1373` keeps a dismissed-but-active Hub job visible; the real gap is *actionability* — see NEW-1.
...
### NEW-14 · P1 — A cross-architecture fine-tune silently discards the base weights and trains a randomly-initialized policy that records itself as a fine-tune
...
### NEW-15 · P2 — A user-requested stop is recorded as a failure, indistinguishable from a crash
...
### NEW-16 · P2 (minor, deprioritized) — Weights & Biases support needs restoring; a cloud resume runs wandb without an API key
...
### NEW-17 · P1 — The cloud checkpoint watcher uploads mid-save snapshots and marks them done forever: Hub checkpoints can be permanently incomplete (unresumable)
...
### NEW-18 · P1 — Switching a local Continue to Hugging Face Cloud silently restarts from step 0 wearing a resume label
...
checkpoint first (bigger). Related: NEW-16 (the W&B resume gap in the same area — additional evidence from
