...
*Renumbered 2026-08-02 into one continuous per-module sequence: the ten confirmed bugs now lead with their RANKING IDs **C1–C10** (bracketed re-verify tags kept), and the former appended `NEW-n` / `N<n>` series became **C11–C19** (NEW-1→C11 … NEW-9→C19); the three design gaps (RANKING D1–D3) are unchanged. Every bug entry now carries a **First identified** date (and **Resolved**, where the entry attests a fix or a descope). Entries may also carry a **Fix:** line, directly under **First identified**, linking the pull request(s) that fix or attempt to fix them.*
...
>   **#7 (T4, cross-feature mutex incl. calibration/auto-cal/wiggle)** overlaps entry 11 / C12, and
>   **#11 (I4, atomic check-and-claim in `start_calibration` + exit guard)** overlaps C4. Neither is on
...
>   findings used the **NEW-n** prefix — *renumbered 2026-08-02: the confirmed-bug headings now lead
>   with their RANKING IDs **C1–C10**, and NEW-1…NEW-9 became **C11–C19**; the three design gaps
>   (RANKING D1–D3) are unchanged. See the note at the top of this file.*
...
The original audit found no P0. **Re-verification adds one: C11 (P0)** — auto-calibration wipes the servos' persistent calibration registers before measuring, with no rollback on Stop/exception. This is **CURRENT P0-1**. See New findings below.

### C1 · P1 — Failed or stopped follower auto-recalibration can delete the previous valid calibration — In progress   [Re-verify entry 1 · RANKING C1]
...
### C2 · P1 — Canceling manual calibration does not restore persistent servo calibration state — In progress   [Re-verify entry 2 · RANKING C2]

**First identified:** 2026-07-14
**Fix:** [PR #31](https://github.com/makermods-robotics/makermodslab/pull/31) (draft) · branch `fix/c2-calibration-cancel-homing-offset-rollback`
...
> — no register restore, no `warning` field. Compounds C11 (auto-cal has the same no-rollback hole on the
> primary path).
...
### C3 · P1 — Manual calibration permits saving grossly incomplete ranges   [Re-verify entry 3 · RANKING C3]
...
### C4 · P1 — Manual calibration lifecycle can overlap worker threads on one singleton   [Re-verify entry 4 · RANKING C4]

**First identified:** 2026-07-14
**Fix:** [PR #11](https://github.com/makermods-robotics/makermodslab/pull/11) (closed) · branch `fix/i4-calibration-atomic-exclusion-and-exit-guard` · [PR #24](https://github.com/makermods-robotics/makermodslab/pull/24) (open) · branch `fix/i4-calibration-atomic-exclusion-and-exit-guard`
...
### C5 · P2 — Concurrent robot-record updates can lose independent fields or collide on the temporary file   [Re-verify entry 5 · RANKING C5]
...
### C6 · P2 — Auto-calibration can report success and assign a missing calibration file   [Re-verify entry 6 · RANKING C6]
...
### C7 · P2 — Imported calibration validation accepts unusable SO-101 profiles   [Re-verify entry 7 · RANKING C7]
...
### C8 · P2 — Browser navigation silently discards unsaved configuration drafts   [Re-verify entry 8 · RANKING C8]
...
### C9 · P2 — Duplicate camera names cause a configured camera to disappear from recording   [Re-verify entry 9 · RANKING C9]
...
### C10 · P3 — Offline station mode labels cached authentication as "HF not configured"   [Re-verify entry 10 · RANKING C10]
...
### C11 · P0 — A stopped or failed auto-calibration leaves the servos' persistent calibration registers wiped, with no rollback *(formerly NEW-1)*

**First identified:** 2026-07-23

> This is **CURRENT P0-1** (the top-ranked P0), compounding C1.
...
**Why this is P0 rather than a duplicate of C2:**
...
4. Combined with C1 (which deletes the follower's calibration *file* on the very same stop), a single cancelled recalibration can destroy **both** the file and the servo state.
...
### C12 · P1 — Nothing prevents starting teleoperation, recording, or inference while arms are auto-calibrating under torque *(formerly NEW-2)*
...
### C13 · P1 — Calibration write-back bypasses the duplicate-port guard, and the resulting record still reports `is_clean: true` *(formerly NEW-3)*
...
### C14 · P1 — Camera identity binds to the browser `deviceId` via fuzzy name matching; the backend's stable `unique_id` is discarded *(formerly NEW-4)*
...
### C15 · P2 — `clamp_motor_power` raises on NaN/Infinity, contradicting its documented "never raises" contract, and can hide every robot from the UI *(formerly NEW-5)*
...
### C16 · P2 — Auto-calibrated leader runs leave a permanent scratch copy in `robots/so_leader/` *(formerly NEW-6)*
...
### C17 · P2 — `ports/{leader,follower}_port.txt` is dead persistence, and `/robot-port/{robot_type}` has no caller *(formerly NEW-7)*
...
### C18 · P2 — The `lelab_biso` → `makermodslab_biso` staging rename has no migration, unlike its two siblings *(formerly NEW-8)*
...
anything walking the cache — the same shape as **C16**'s scratch-copy accumulation.
...
**Update 2026-08-03 (F6 session):** the live-install question is answered — `ls ~/.cache/huggingface/lerobot/`
today shows **`makerlab_biso/` (the INTERMEDIATE rename hop) and no `makermodslab_biso/`**, so this machine's
staged copies are orphaned right now and the dir will be recreated fresh on next bimanual session. Two
corrections this implies: (1) the fix must walk the full three-name chain (`lelab_biso` → `makerlab_biso` →
`makermodslab_biso`), not just the oldest name — same shape as the `_local_models_root` gap fixed in the
parked `rename-gap.patch` (session scratchpad), which added `_LEGACY_MODELS_DIR_NAMES` and is the pattern to
mirror here; (2) the entry's 2026-07-24 listing claim above ("`makermodslab_biso/` present") is almost
certainly a **rename-sweep artifact** — on 07-24 the code's current name was still `makerlab_biso`, and the
rebrand's text sweep appears to have rewritten the historical observation along with the code references
(matches the known path-blind sweep failure mode from the 08-03 rebase notes). Impact stays bounded
(staging is rewritten unconditionally each session; orphaned disk, not lost data).

**Update 2026-08-03, later:** this machine's orphan is resolved by hand — `makerlab_biso/` was `mv`'d to
`makermodslab_biso/` (user-directed). The CODE gap (no migration walk for any install that still carries a
legacy name) remains open and is what this entry now tracks.

---

### C19 · P2 — The Quality workflow has failed on `main` for days and gives no signal, because it lints the whole repo including vendored kernel source *(formerly NEW-9)*
...
### C20 · P2 — `_STOP_GRACE_S = 18.0` hand-sums the vendored autocal script's stop budget; nothing pins the invariant against drift

**First identified:** 2026-08-02

**Files:** `makermodslab/auto_calibrate.py:52-64`; `makermodslab/vendor/feetech_autocal/auto_calibrate_script.py` (`RETURN_TO_REST_BUDGET_S`, `STOP_HOLD_S`)

`_STOP_GRACE_S = 18.0` is a hand-computed literal: its own comment derives it as the sum of the vendored script's SIGTERM cleanup budget (~1s freeze + `RETURN_TO_REST_BUDGET_S` 10s + `STOP_HOLD_S` 3s + 1–2s release/disconnect ≈ 15–16s) plus margin, and states the load-bearing invariant — the grace must stay **above** the script's summed stop budget so the SIGKILL escalation can never land on a healthy graceful stop mid-motion (i.e. mid return-to-rest, arm under torque). But the inputs live in the vendored file and nothing enforces the relationship: bumping the vendored return budget silently invalidates `18.0` and turns the escalation into a kill-during-healthy-stop. Same hand-mirrored-twin hazard `rest_pose.py` already documents against this vendor file. Fix is cheap and behavior-preserving: import the vendored constants and compute the sum (as `_STOP_AND_WAIT_CEILING_S` already does for its own layer, PR #28), or at minimum add a test asserting `_STOP_GRACE_S > vendored_sum`. Noted while reviewing PR #28 (I7); deliberately kept out of that PR — the constant and its derivation pre-date the shutdown-fix lineage.
...
> See **C12** for the reachable consequence. PR #7 (`fix/t4-cross-feature-mutex`) adds calibration/auto-cal/
